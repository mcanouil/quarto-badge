...
| --- | --- | --- | --- |
| `key` | string | Yes | Selects the badge configuration to use. Matches the `key` of one entry under `extensions.badge.badges`. |
...
Badges are defined under `extensions.badge.badges`, in `_quarto.yml` for a whole project or in a document's front matter for a single page.
...
Every property applies to one entry of the `extensions.badge.badges` array.
...
`extensions.badge.overrides` redefines project-level badges for a single document.
...
The overrides take the same options as `extensions.badge.badges`.
...
Overrides are read from `extensions.badge.overrides`.
...
| `key` (shortcode) | A key defined under `extensions.badge.badges` or added by `extensions.badge.overrides`. | The shortcode renders as nothing. |
